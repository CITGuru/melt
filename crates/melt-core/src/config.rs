...
/// login upstream; `Seed` short-circuits the login and serves a
/// canned demo fixture so an operator can run Melt with no Snowflake
/// account. See `docs/SEED_MODE.md` and KI-002.
...
impl SessionMode {
    pub fn is_seed(self) -> bool {
        matches!(self, SessionMode::Seed)
    }

    pub fn as_str(self) -> &'static str {
        match self {
            SessionMode::Real => "real",
            SessionMode::Seed => "seed",
        }
    }
}
...
///
/// `mode = "real"` (default) preserves the production login path —
/// every `POST /session/v1/login-request` is forwarded upstream and
/// the resulting session token is registered in the in-memory
/// `SessionStore`.
///
/// `mode = "seed"` is the credential-free demo path (KI-002). The
/// proxy never contacts upstream Snowflake: logins matching the
/// canned demo creds (`account=melt-demo, user=demo, password=demo`)
/// receive a synthetic Snowflake-shaped response signed by a fixed
/// seed token, and statements run against a local DuckDB fixture
/// (default `var/melt/seed.ddb`). Anything that genuinely needs
/// upstream Snowflake (writes, INFORMATION_SCHEMA, oversize lake
/// tables) returns `MeltError::SeedModeUnsupported`.
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct SessionsConfig {
    #[serde(default)]
    pub mode: SessionMode,

    /// Path to the single-file DuckDB used by seed mode. Generated by
    /// `melt sessions seed --init`. Ignored in real mode. Default
    /// `var/melt/seed.ddb`.
    #[serde(default = "SessionsConfig::default_fixture_path")]
    pub fixture_path: PathBuf,
}

impl SessionsConfig {
    fn default_fixture_path() -> PathBuf {
        PathBuf::from("var/melt/seed.ddb")
    }
}
...
impl Default for SessionsConfig {
    fn default() -> Self {
        Self {
            ...
            fixture_path: Self::default_fixture_path(),
        }
    }
}

/// Canonical demo credentials baked into seed mode. Operators don't
/// override these — the goal is "git clone → working query in five
/// minutes" with one universally documented set of creds. Anyone who
/// needs different creds is using real mode.
pub const SEED_ACCOUNT: &str = "melt-demo";
pub const SEED_USER: &str = "demo";
pub const SEED_PASSWORD: &str = "demo";
pub const SEED_DATABASE: &str = "TPCH";
pub const SEED_SCHEMA: &str = "SF01";
pub const SEED_WAREHOUSE: &str = "MELT_DEMO_WH";
pub const SEED_ROLE: &str = "PUBLIC";
pub const SEED_TOKEN: &str = "melt-seed-mode-token";
